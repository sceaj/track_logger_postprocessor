...
class CanFrame14AExtractor(object):
    ...
    def extractData(self, field_data, state):
        psm_disable = CanFrameParser.frame_byte(field_data, 1) & 0x20
        state.set_data_item(DataState.names[DataState.names.PSM_Disable], psm_disable == 0x20)
        print("PSM_Disable: {0}".format(psm_disable == 0x20))
 
class CanFrame242Extractor(object):
    
    def extractData(self, field_data, state):
        if (CanFrameParser.frame_byte(field_data, 0) & 0x08) == 0x08:
            clutch_pedal = "Depressed"
        else:
            clutch_pedal = "Released"
...
class CanFrame245Extractor(object):
    ...
    def extractData(self, field_data, state):
        ...
        if (CanFrameParser.frame_byte(field_data, 2) & 0x02) == 0x02:
            brake_pedal = "Applied"
        else:
            brake_pedal = "Released"
...
class CanFrame308Extractor(object):
    ...
    def extractData(self, field_data, state):
        sport_mode = CanFrameParser.frame_byte(field_data, 0) & 0x20
        state.set_data_item(DataState.names[DataState.names.Sport_Mode], sport_mode == 0x20)
        print("Sport Mode: {0}".format(sport_mode == 0x20))
...
class CanFrame442Extractor(object):
    ...
    def extractData(self, field_data, state):
        pasm_sport_mode = (CanFrameParser.frame_byte(field_data, 0) & 0x11)
        state.set_data_item(DataState.names[DataState.names.Pasm_Sport_Mode], pasm_sport_mode == 0x11)
        print("PASM Sport Mode: {0}".format(pasm_sport_mode == 0x11))
